...
export const StoriesLibraryView: React.FC<StoriesLibraryViewProps> = ({
  ...
  return (
    <div className="space-y-8 pb-16" dir="rtl">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-white dark:bg-slate-900 p-4 rounded-2xl border border-amber-200 dark:border-slate-800 shadow-sm transition-colors">
        {/* Category Pills */}
            ...
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-bold transition-all shrink-0 ${
                selectedCategory === cat.id
                  ...
                  : 'bg-slate-100 dark:bg-slate-800 hover:bg-amber-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300'
              }`}
            >
              {cat.label}
            </button>
        ...
        <div className="relative w-full md:w-80">
          ...
          <input
            ...
            className="w-full pr-10 pl-4 py-2 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 focus:bg-white dark:focus:bg-slate-800 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-200 text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 text-xs font-medium outline-none transition-colors"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
        {filteredProphets.map(p => {
          ...
          return (
            <div
              key={p.id}
              className="group bg-white dark:bg-slate-900 rounded-3xl p-5 border-2 border-amber-200 dark:border-slate-800 shadow-sm hover:shadow-xl hover:border-emerald-400 dark:hover:border-emerald-500 transition-all duration-300 flex flex-col justify-between"
            >
              <div className="space-y-4">
                ...
                <div>
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[11px] font-bold text-amber-800 dark:text-amber-300 bg-amber-100 dark:bg-amber-950/80 px-2 py-0.5 rounded-md">
                      ذكر {p.quranicMentionsCount} مرة
                    </span>
                  </div>

                  <p className="text-xs font-bold text-emerald-800 dark:text-emerald-400 mt-1 line-clamp-1">
                    {p.title}
                  </p>
                ...
                <div className="space-y-1 pt-1 border-t border-slate-100 dark:border-slate-800">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {p.coreValues.map(v => (
                      <span
                        key={v.id}
                        className="px-2 py-0.5 rounded-md bg-emerald-50 dark:bg-emerald-950/70 text-emerald-800 dark:text-emerald-300 text-[10px] font-semibold border border-emerald-100 dark:border-emerald-800/80"
                      >
                        {v.title}
                      </span>
                    ))}
              ...
              <div className="pt-4 mt-4 border-t border-slate-100 dark:border-slate-800 flex items-center gap-2">
                ...
                <button
                  onClick={() => onSelectStory(p.id)}
                  className="p-2.5 rounded-xl bg-amber-100 dark:bg-slate-800 hover:bg-amber-200 dark:hover:bg-slate-700 text-amber-900 dark:text-amber-300 transition-colors"
                  title="استمع للقصة"
                >
